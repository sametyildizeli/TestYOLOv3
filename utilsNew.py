...
def convertToNumpyArray(list_, intOrNot):
    ...
    if intOrNot == "float": 
        ...
        nArray = np.asarray(arry, dtype=np.float32)
    elif intOrNot == "int":
        for i in list_:
            for j in i:
                arry.append(j)      

        nArray = np.asarray(arry, dtype=np.int32)

    return nArray

def ap_per_class_new(tp, conf, pred_cls, target_cls):
    ...
    tp_ = convertToNumpyArray(tp, "float")
    conf_ = convertToNumpyArray(conf, "float")
    pred_cls_ = convertToNumpyArray(pred_cls, "int")
    target_cls_ = convertToNumpyArray(target_cls, "int")
    print("tp_:")
    print(tp_)
    print("conf_ :")
    print(conf_)
    ...
    tp_, conf_, pred_cls_ = tp_[i], conf_[i], pred_cls_[i]
    print("***********************************************************")
    print("tp_:")
    print(tp_)
    print("conf_ :")
    ...
    unique_classes = np.unique(target_cls_)
    ...
    for c in unique_classes:
        i = pred_cls_ == c
        print(i)
        n_gt = (target_cls_ == c).sum()  # Number of ground truth objects
...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for c in unique_classes:
        ...
        if (n_p == 0) and (n_gt == 0):
            continue
        elif (n_p == 0) and (n_gt == 1):
            ap.append(0)
            r.append(0)
            p.append(0)
        elif (n_p == 1) and (n_gt ==0):
            ap.append(0)
            r.append(0)
            p.append(1)
        else:
